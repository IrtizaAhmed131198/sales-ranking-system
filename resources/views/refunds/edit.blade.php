...
@section('content')
    <div class="container-fluid p-0">
        <div class="d-flex justify-content-between align-items-center mb-4">
            <div>
                <h2 class="fw-bold m-0">Edit Refund Entry</h2>
                <p class="text-secondary m-0">Modify an existing monthly refund deduction</p>
            </div>
                ...
                <form action="{{ route('refunds.update', $refund->id) }}" method="POST">
                    ...
                    <div class="mb-3">
                        ...
                        <select name="user_id" id="user_id" class="form-select ajax-select" required>
                            ...
                            @php
                                $selectedUserId = old('user_id', $refund->user_id);
                    ...
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            ...
                            <input type="month" name="refund_month" id="refund_month" class="form-control" value="{{ old('refund_month', date('Y-m', strtotime($refund->refund_month))) }}" required>
                            @error('refund_month')
                                <div class="text-danger small mt-1">{{ $message }}</div>
                            @enderror
                        </div>
                        <div class="col-md-6 mb-4">
                            ...
                            <div class="input-group">
                                ...
                                <input type="number" step="0.01" min="0.01" name="amount" id="amount" class="form-control" placeholder="0.00" value="{{ old('amount', $refund->amount) }}" required>
                            </div>
                            @error('amount')
                                <div class="text-danger small mt-1">{{ $message }}</div>
                            @enderror
                    ...
                    <div class="d-flex justify-content-end">
                        <button type="submit" class="btn btn-primary fw-semibold px-4">Update Refund</button>
                    </div>
                </form>
            </div>
        </div>
