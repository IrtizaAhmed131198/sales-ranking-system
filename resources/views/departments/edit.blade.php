        ...
        <div class="card-body p-4">
            <form action="{{ route('departments.update', $department->id) }}" method="POST">
                ...
                <div class="mb-3">
                    ...
                    <input type="text" name="head_name" id="head_name" class="form-control @error('head_name') is-invalid @enderror" value="{{ old('head_name', $department->head_name) }}" required placeholder="e.g. John Doe">
                    @error('head_name')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="mb-4">
                    ...
                    <input type="number" step="0.01" name="target" id="target" class="form-control @error('target') is-invalid @enderror" value="{{ old('target', $department->target) }}" placeholder="e.g. 500000.00" min="0">
                    @error('target')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
